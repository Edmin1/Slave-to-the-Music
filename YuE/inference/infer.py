...
tokenizer_path = os.path.join(os.path.dirname(__file__), "mm_tokenizer_v0.2_hf", "tokenizer.model")
...
torch.cuda.empty_cache()  # free up memory before loading
model_stage2 = AutoModelForCausalLM.from_pretrained(
    stage2_model,
    torch_dtype=torch.float16,
    device_map="auto",  # let HF offload automatically
    low_cpu_mem_usage=True
)
model_stage2.eval()
model_stage2.eval()
...
def stage2_inference(model_stage2, data_array_list, file_name_list, stage2_output_dir, batch_size=4):
    for i, arr in enumerate(data_array_list):
        output_filename = os.path.join(stage2_output_dir, os.path.basename(file_name_list[i]))
        ...
        if prompt.size == 0:
            print(f"Skipping empty prompt file: {stage1_output_set[i]}")
            continue
        ...
        if num_batch <= batch_size:
            ...
            print(f"[DEBUG] prompt shape: {prompt.shape}, output_duration: {output_duration}, slice: {output_duration*50}")
...
print("[DEBUG] Stage 1 output is a list with length:", len(stage1_output_set))

new_stage1_output = []
for idx, item in enumerate(stage1_output_set):
    print(f"[DEBUG] Segment {idx} raw value:", item)

    if isinstance(item, str):
        try:
            # Try converting space-separated string of integers into a numpy array
            arr = np.load(item)
            print(f"[INFO] Converted Segment {idx} to array: shape {arr.shape}, size {arr.size}")
            if arr.size == 0:
                print(f"[WARNING] Segment {idx} is empty after conversion!")
            new_stage1_output.append(arr)
        except Exception as e:
            print(f"[ERROR] Failed to convert Segment {idx}: {e}")
            raise
    else:
        print(f"[WARNING] Segment {idx} is not a string. Type: {type(item)} — skipping conversion.")
        new_stage1_output.append(item)

stage1_output_set = new_stage1_output
