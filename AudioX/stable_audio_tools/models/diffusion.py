...
class ConditionedDiffusionModelWrapper(nn.Module):
    ...
    def __init__(
            ...
            model_config: tp.Optional[dict] = None,
            ):
        super().__init__()
    ...
    def get_conditioning_inputs(self, conditioning_tensors: tp.Dict[torch.Tensor, tp.Any], negative=False):
        ...
        if len(self.cross_attn_cond_ids) > 0:
            ...
            for key in self.model_config["diffusion"]["cross_attention_cond_ids"]:
                if key not in conditioning_tensors:
                    continue

                x, m = conditioning_tensors[key]
                ...
                cross_attention_input.append(x)
                cross_attention_masks.append(m)

            if len(cross_attention_input) == 0:
                raise ValueError("No valid conditioning tensors found for cross-attention inputs. Did you forget to include 'text_prompt'?")
